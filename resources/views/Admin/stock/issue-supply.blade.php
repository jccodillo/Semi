        ...
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-header pb-0">
                    <h6>Issue Supply</h6>
                    <p class="text-sm">Decrease quantity for: <strong>{{ $supply->product_name }}</strong></p>
                </div>
                <div class="card-body">
                    @if(session('error'))
                    ...
                    <form action="{{ route('admin.supplies.process-issuance', $supply->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    <label for="quantity" class="form-control-label">Quantity to Issue</label>
                                    <div class="input-group">
                                        <input type="number" class="form-control" id="quantity" name="quantity" 
                                               value="{{ old('quantity', 1) }}" min="1" max="{{ $supply->quantity }}" required>
                                    ...
                                    <small class="form-text text-muted">Enter the number of units you want to issue from the current stock ({{ $supply->quantity }}).</small>
                                    @error('quantity')
                                ...
                                <div class="form-group mb-3">
                                    <label for="office" class="form-control-label">Issuing Office/Department</label>
                                    <input type="text" class="form-control" id="office" name="office" value="{{ old('office') }}" required>
                                    <small class="form-text text-muted">Enter the name of the office or department receiving this supply.</small>
                                    @error('office')
                                        <span class="text-danger text-xs">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <div class="d-flex justify-content-end mt-4">
                            ...
                            <button type="submit" class="btn btn-primary">Issue Supply</button>
                        </div>
                    </form>
                </div>
            </div>
